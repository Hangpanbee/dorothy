...
def reply():
    try:
        if request.methods == 'POST':
            message = request.form['sentimental_text']
            #soup = BeautifulSoup(html_doc, "Chatbot.html")
            #message = soup.find_all("div", id ='text')
            a = TextBlob(message).sentiment.polarity
            b = TextBlob(message).sentiment.subjectivity
            if float(a) >= 0.5 and float(b) >= 0.5:
                answer = "you seem really happy today. You wanna share some good news about you and your friend? "
                a = jsonify(text = answer)
                return jsonify(text = answer) 
            else:
                answer = 'yikes'
                return jsonify(text = 'yikes')
            
            #else:
                #return jsonify(text = 'Ooops please restart me and the session. I am so sorry!')
    
    except:
        #time.sleep(5)
        return jsonify(text = 'uh oh. That sounds difficult. Do you want to explain it in simpler term? It helps!')
'''
...
def reply():
    ...
    try:
        ...
        if float(a) >= 0.3 and float(a) < 0.5 and float(b) < 0.5:
            ...
            list_of_greet = ['bye', 'goodbye', 'see you again', 'see you soon', 'goodnight', 'night', 'morning', 'good morning', "g'day mate"]
            ...
            if message in list_of_greet:
                ...
                output = make_pred(sess, input_data, input_data_len, target_data_len, keep_prob, mode_input, batch_size, logits, index_to_vocabs)
